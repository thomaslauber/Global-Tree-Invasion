...
outputPath = 'results'
dt = '2022-05-16'#datetime.datetime.today().strftime('%Y-%m-%d')
...
input_data['lgsprich'] = input_data['sprich'].apply(lambda x: math.log(x))
...
native_prob_inv.columns = [classificationVariable] + ['Species Richness (log)', 
...
invasion_prop_nat.columns = [classificationVariable] + ['Species Richness (log)', 
                                                      'Distance to Ports', 'Population Density',
               'Annual Mean Temperature', 'Annual Precipitation', 
               'Absolute depth to bedrock', 'Coarse Fragments', 
               'Sand Content', 'Silt Content', 'Soil pH H2O']
...
features = invasion_prop_nat.drop([classificationVariable], axis = 1)
feature_list = list(features.columns)
features = np.array(features)
features_df = pd.DataFrame(data=features, columns=feature_list)
train_features, test_features, train_labels, test_labels = train_test_split(features, labels, test_size = 0.25, random_state = 42)
...
kFold_rSquared = 0.6943881137903889
params = pd.read_csv(outputFolder + '/' + output_name + '_CV_bestParams.csv')
...
# mean shap 
plt.figure()
shap.summary_plot(shap_values, pd.DataFrame(data=test_features, columns=feature_list), plot_type="bar", show=False, color='#440154', sort=False)
current_handles, current_labels = plt.gca().get_legend_handles_labels()
plt.legend(current_handles, ['invaded', 'not invaded'])
plt.xlabel('Mean absolute SHAP value')
plt.yticks([])
...
plt.figure()
shap.summary_plot(shap_values, pd.DataFrame(data=test_features, columns=feature_list), show=False, sort=False)
plt.xlabel('SHAP value')
for fc in plt.gcf().get_children():
    for fcc in fc.get_children():
        if hasattr(fcc, "set_cmap"):
...
Shap value tables
"""
shap_df = pd.Series(pd.DataFrame(shap_values, columns=feature_list).abs().mean(0), name = 'non-invaded')
shap_df.to_csv(outputFolder + '/' + output_name + '_shap_values.csv', sep=';', index=False)
...
invasion_prop_ba_nat.columns = [classificationVariable] + ['Species Richness (log)', 
                                                      'Distance to Ports', 'Population Density',
               'Annual Mean Temperature', 'Annual Precipitation', 
               'Absolute depth to bedrock', 'Coarse Fragments', 
               'Sand Content', 'Silt Content', 'Soil pH H2O']
...
features = invasion_prop_ba_nat.drop([classificationVariable], axis = 1)
feature_list = list(features.columns)
features = np.array(features)
features_df = pd.DataFrame(data=features, columns=feature_list)
train_features, test_features, train_labels, test_labels = train_test_split(features, labels, test_size = 0.25, random_state = 42)
...
kFold_rSquared = 0.2590287316238583
params = pd.read_csv(outputFolder + '/' + output_name + '_CV_bestParams.csv')
