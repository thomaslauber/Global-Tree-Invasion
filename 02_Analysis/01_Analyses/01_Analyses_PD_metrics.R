...
phylo.met <- dat %>%                                                                                         # Start with dat
  select(c("nat.faith","nat.mntd","nat.mpd","nat.vntd","nat.vpd","lgsprich"))                                # Subset to cols of interest
phylo.cor <- as.matrix(cor(phylo.met))                                                                       # Create matrix to investigate colinearity

env.simp2 <- dat %>% 
  select(c("CHELSA_BIO_Annual_Mean_Temperature","CHELSA_BIO_Annual_Precipitation",
           "SG_Absolute_depth_to_bedrock",   "SG_Bulk_density_000cm",          "SG_CEC_000cm",                  
           "SG_Clay_Content_000cm",          "SG_Coarse_fragments_000cm",      "SG_H2O_Capacity_000cm",         
           "SG_SOC_Content_000cm",         "SG_Sand_Content_000cm",          "SG_Saturated_H2O_Content_000cm",
           "SG_Silt_Content_000cm" ,         "SG_Soil_pH_H2O_000cm"))
env.simp.cor <- as.matrix(cor(na.omit(env.simp2))) 
...
temp.cor <- as.matrix(cor(na.omit(temp))) 

#TROP
trop <- dat %>% 
  select(c("CHELSA_BIO_Annual_Mean_Temperature","CHELSA_BIO_Annual_Precipitation",
           "SG_Absolute_depth_to_bedrock", "SG_SOC_Content_000cm",  "SG_Soil_pH_H2O_000cm"))
trop.cor <- as.matrix(cor(na.omit(trop)))
...
saveRDS(dat.stat.mod.noscale,"data/RACdf_noscale_global_dat.stat.mod.rds")
...
png("figures/PD/dat.stat.int.jpg", width = 6, height = 6, units = 'in', res = 300)
plot(datplot)
dev.off()
...
dat.prop.mod.noscale <- dat.prop.mod.noscale %>%
  mutate(MAT = MAT/10, MAP = MAP/10)
saveRDS(dat.prop.mod.noscale,"data/RACdf_noscale_global_dat.prop.mod.rds")
...
dat.prop.modb.noscale <- dat.prop.modb.noscale %>%
  mutate(MAT = MAT/10, MAP = MAP/10)
saveRDS(dat.prop.modb.noscale,"data/RACdf_noscale_global_dat.prop.modb.rds")
...
# Colonization
# Run non-binomial version for spat auto
mod.col <- glm((invasct/sprich)~ nat.faith + nat.mntd +    
                 dist.ports + popdensity + MAT + MAP + 
                 SG_Absolute_depth_to_bedrock + SG_Coarse_fragments_000cm +
                 SG_Sand_Content_000cm + SG_Silt_Content_000cm + SG_Soil_pH_H2O_000cm, 
                 weights= sprich, data = dat.prop.mod)

my_comb <- dat.prop.mod %>%
  mutate(ypred = predict(mod.col)) %>%
  mutate(resids = propinvasive - ypred) 

sp.col <- ncf::spline.correlog(x = as.numeric(my_comb$avglon),
                               y = as.numeric(my_comb$lat),
                               z = as.numeric(my_comb$resids),
                               xmax = 500, resamp = 100, latlon=TRUE) 

# Spread
my_comb <- dat.prop.mod.b %>%
  mutate(ypred = predict(mod.spread)) %>%
  mutate(resids = pinvba - ypred) 

sp.spread <- ncf::spline.correlog(x = as.numeric(my_comb$avglon),
                                  y = as.numeric(my_comb$lat),
                                  z = as.numeric(my_comb$resids),
                                  xmax = 500, resamp = 100, latlon=TRUE) 

png("figures/spat.auto_phylo.jpg", width = 6, height = 10, units = 'in', res = 300)
par(mfrow = c(2, 1))
plot(sp.col,main="col", ylim=c(-0.25, 1))
plot(sp.spread,main="spread", ylim=c(-0.25, 1))
dev.off()
...
rac <- Spat.cor(mod.stat.sp,dat.stat.mod,250)
dat.stat.mod$rac <- rac
dat.stat.mod.noscale$rac <- rac
dat.stat.mod.noscale <- dat.stat.mod.noscale %>%
  mutate(MAT = MAT/10, MAP = MAP/10)
saveRDS(dat.stat.mod.noscale,"data/RACdf_noscale_natsprich_global_dat.stat.mod.rds")
...
# Temperate
mod.stat.sp.temp <- glm(instatus ~ nativect +    
                       dist.ports + popdensity + MAT + MAP + 
                       SG_Absolute_depth_to_bedrock + SG_Clay_Content_000cm + SG_Soil_pH_H2O_000cm, 
                       family = binomial,data = dat.stat.mod)

rac <- Spat.cor(mod.stat.sp.temp,dat.stat.mod,250)
dat.stat.mod$rac <- rac
...
# Tropical
mod.stat.sp.trop <- glm(instatus ~ nativect +    
                       dist.ports + popdensity + MAT + MAP + 
                       SG_Absolute_depth_to_bedrock + SG_SOC_Content_000cm + SG_Soil_pH_H2O_000cm,
                       family = binomial,data = dat.stat.mod)

rac <- Spat.cor(mod.stat.sp.trop,dat.stat.mod,250)
dat.stat.mod$rac <- rac
...
# Global
mod.col.sp <- glm(cbind(invasct,nativect) ~ nativect +    
                 dist.ports + popdensity + MAT + MAP + 
                 SG_Absolute_depth_to_bedrock + SG_Coarse_fragments_000cm +
                 SG_Sand_Content_000cm + SG_Silt_Content_000cm + SG_Soil_pH_H2O_000cm, 
               family = binomial, data = dat.prop.mod)

rac <- Spat.cor(mod.col.sp,dat.prop.mod,250)
dat.prop.mod$rac <- rac
dat.prop.mod.noscale$rac <- rac
dat.prop.mod.noscale <- dat.prop.mod.noscale %>%
  mutate(MAT = MAT/10, MAP = MAP/10)
saveRDS(dat.prop.mod.noscale,"data/RACdf_noscale_natsprich_global_dat.prop.mod.rds")
...
# Temperate
mod.col.sp.temp <- glm(cbind(invasct,nativect) ~ nativect +    
                      dist.ports + popdensity + MAT + MAP + 
                      SG_Absolute_depth_to_bedrock + SG_Clay_Content_000cm + SG_Soil_pH_H2O_000cm, 
                    family = binomial, data = dat.prop.mod)

rac <- Spat.cor(mod.col.sp.temp,dat.prop.mod,250)
dat.prop.mod$rac <- rac
...
# Tropical
mod.col.sp.trop <- glm(cbind(invasct,nativect) ~ nativect +    
                      dist.ports + popdensity + MAT + MAP + 
                      SG_Absolute_depth_to_bedrock + SG_SOC_Content_000cm + SG_Soil_pH_H2O_000cm,
                    family = binomial, data = dat.prop.mod)

rac <- Spat.cor(mod.col.sp.trop,dat.prop.mod,250)
dat.prop.mod$rac <- rac
...
dat.prop.mod.b <- dat.prop.mod %>% filter(!pinvba == 1 & !pinvba == 0)
# Global
mod.spread.sp <- betareg(pinvba ~ nativect +    
                        dist.ports + popdensity + MAT + MAP + 
                        SG_Absolute_depth_to_bedrock + SG_Coarse_fragments_000cm +
                        SG_Sand_Content_000cm + SG_Silt_Content_000cm + SG_Soil_pH_H2O_000cm, 
                      data = dat.prop.mod.b)

rac <- Spat.cor(mod.spread.sp,dat.prop.mod.b,250)
dat.prop.mod$rac <- rac
dat.prop.modb.noscale$rac <- rac
dat.prop.modb.noscale <- dat.prop.modb.noscale %>%
  mutate(MAT = MAT/10, MAP = MAP/10)
saveRDS(dat.prop.modb.noscale,"data/RACdf_noscale_natsprich_global_dat.prop.modb.rds")

mod.spread.sp <- glm(cbind(invasct,nativect) ~ nativect +    
                    dist.ports + popdensity + MAT + MAP + 
                    SG_Absolute_depth_to_bedrock + SG_Coarse_fragments_000cm +
                    SG_Sand_Content_000cm + SG_Silt_Content_000cm + SG_Soil_pH_H2O_000cm + rac, 
                  family = binomial, data = dat.prop.mod.b)
...
# Temperate
mod.spread.sp.temp <- betareg(pinvba ~ nativect +    
                             dist.ports + popdensity + MAT + MAP + 
                             SG_Absolute_depth_to_bedrock + SG_Clay_Content_000cm + SG_Soil_pH_H2O_000cm, 
                           data = dat.prop.mod.b)

rac <- Spat.cor(mod.spread.sp.temp,dat.prop.mod.b,250)
dat.prop.mod.b$rac <- rac

mod.spread.sp.temp <- glm(cbind(invasct,nativect) ~ nativect +    
                         dist.ports + popdensity + MAT + MAP + 
                           SG_Absolute_depth_to_bedrock + SG_Clay_Content_000cm + SG_Soil_pH_H2O_000cm + rac, 
                       family = binomial, data = dat.prop.mod.b)

# Tropical
mod.spread.sp.trop <- betareg(pinvba ~ nativect +    
                             dist.ports + popdensity + MAT + MAP + 
                             SG_Absolute_depth_to_bedrock + SG_SOC_Content_000cm + SG_Soil_pH_H2O_000cm,
                           data = dat.prop.mod.b)

rac <- Spat.cor(mod.spread.sp.trop,dat.prop.mod.b,250)
dat.prop.mod.b$rac <- rac

mod.spread.sp.trop <- glm(cbind(invasct,nativect) ~ nativect +    
                         dist.ports + popdensity + MAT + MAP + 
                           SG_Absolute_depth_to_bedrock + SG_SOC_Content_000cm + SG_Soil_pH_H2O_000cm + rac, 
                       family = binomial, data = dat.prop.mod.b)

summary(mod.spread.sp)
...
# Colonization
my_comb <- dat.prop.mod %>%
  mutate(ypred = predict(mod.col.sp)) %>%
  mutate(resids = propinvasive - ypred) 

sp.col.sp <- ncf::spline.correlog(x = as.numeric(my_comb$avglon),
                               y = as.numeric(my_comb$lat),
                               z = as.numeric(my_comb$resids),
                               xmax = 500, resamp = 100, latlon=TRUE) 
...
# Spread
my_comb <- dat.prop.mod.b %>%
  mutate(ypred = predict(mod.spread.sp)) %>%
  mutate(resids = pinvba - ypred) 

sp.spread.sp <- ncf::spline.correlog(x = as.numeric(my_comb$avglon),
                                  y = as.numeric(my_comb$lat),
                                  z = as.numeric(my_comb$resids),
                                  xmax = 500, resamp = 100, latlon=TRUE) 

png("figures/spat.auto_sprich.jpg", width = 6, height = 10, units = 'in', res = 300)
par(mfrow = c(2, 1))
plot(sp.col.sp,main="col", ylim=c(-0.25,1))
plot(sp.spread.sp,main="spread", ylim=c(-0.25,1))
dev.off()
...
# stat sp better, severity phylo better
AIC(mod.stat,mod.stat.sp)           
AIC(mod.col,mod.col.sp)             
AIC(mod.spread,mod.spread.sp)     

# temp = depends/not that different
AIC(mod.stat.temp,mod.stat.sp.temp)           
AIC(mod.col.temp,mod.col.sp.temp)             
AIC(mod.spread.temp,mod.spread.sp.temp)  

#trop = phylo better than sp
AIC(mod.stat.trop,mod.stat.sp.trop)           
AIC(mod.col.trop,mod.col.sp.trop)             
AIC(mod.spread.trop,mod.spread.sp.trop)  
...
dat.prop.mod.noscale <- dat.prop.mod.noscale %>%
  mutate(MAT = MAT/10, MAP = MAP/10)
saveRDS(dat.prop.mod.noscale,"data/RACdf_noscale_dmntd_global_dat.prop.mod.rds")
...
mod.temp <- glm(d.mntd ~  MAP+ MAT*dist.ports+ nat.faith +  popdensity+
                  SG_Absolute_depth_to_bedrock + SG_Clay_Content_000cm + SG_Soil_pH_H2O_000cm,
                data= dat.prop.mod)
...
dat.prop.mod.noscale <- dat.prop.mod.noscale %>%
  mutate(MAT = MAT/10, MAP = MAP/10)
saveRDS(dat.prop.mod.noscale,"data/RACdf_noscale_dmntd_temp_dat.prop.mod.rds")
...
#                data=dat.prop.mod)
#simplified full version
mod.trop <- glm(d.mntd ~  MAT +MAP+dist.ports+nat.faith+   popdensity+
                  SG_Absolute_depth_to_bedrock + SG_SOC_Content_000cm  + SG_Soil_pH_H2O_000cm,
                data= dat.prop.mod)
...
dat.prop.mod.noscale <- dat.prop.mod.noscale %>%
  mutate(MAT = MAT/10, MAP = MAP/10)
saveRDS(dat.prop.mod.noscale,"data/RACdf_noscale_dmntd_trop_dat.prop.mod.rds")
...
#check relationships
datplot<-ggeffect(mod, terms= c("MAT"))
...
diff.MAT_resid <-
  ...
  scale_y_continuous(labels = label_number(accuracy = 0.1),limits=c(-1.5,1.5))
  
png("figures/PD/Global_MAT.jpg", width = 6, height = 6, units = 'in', res = 300)
diff.MAT_resid
dev.off()
...
resid.lowtemp <- resid %>% filter(MAT < 3.343949 & d.mntd < 0) %>% select(lat, avglon, MAT) %>% mutate(extreme = "low")
resid.hightemp <- resid %>% filter(MAT > 26.35085 & d.mntd < 0) %>% select(lat, avglon, MAT) %>% mutate(extreme = "high")

resid.ext <- rbind(resid.lowtemp,resid.hightemp)
saveRDS(resid.ext, "data/PDstrategy_tempextremes.rds")
...
viridis(option="rocket",12)

colScale <- scale_colour_manual(values =c ("#921C5BFF", "#F6A47BF"))
fillScale <- scale_fill_manual(values =c ("#921C5BFF", "#F6A47BF"))

dist.ports.minmax <- data.frame(dist.ports = quantile(mod$data$dist.ports, c(cut_tol,0.5, 1-cut_tol)), level =c ("low","medium", "high")) 
MAT.range <- seq(from = quantile(dat.prop.mod$MAT, 0.001),to= quantile(dat.prop.mod$MAT, 0.999),length.out = 100) 
ex.grid <- expand.grid(MAT = MAT.range, dist.ports = dist.ports.minmax$dist.ports) 
pred.dat <- cbind(ex.grid,t(colMeans(na.omit(dat.prop.mod[,c("nat.faith", "MAP", "dist.ports", "popdensity",
                                                               "SG_Absolute_depth_to_bedrock", "SG_Clay_Content_000cm", "SG_Soil_pH_H2O_000cm","rac")])))) 
...
diff.MAT_dist.ports_resid <-
  ggplot(data = resid,aes(x = MAT, y = resid.d.mntd, color=dist.ports))+
  geom_point(alpha = 0.35, size =2.5)+
  xlab("Mean Annual Temperature (\u00B0C)")+
  ylab("Invasion Strategy")+
  geom_hline(yintercept = 0, linetype = "dashed")+
  ...
  geom_line(data = pdat.low, aes(x = MAT, y = fit),color="#F6A47BFF") +
  geom_ribbon(data = pdat.low,aes(y=NULL,x=MAT,ymin = fit-se.fit, ymax = fit+se.fit), fill = "#F6A47BFF", alpha = .6,color = NA)+
  geom_line(data = pdat.high, aes(x = MAT, y = fit),color="#701F57FF") +
  geom_ribbon(data = pdat.high,aes(y=NULL,x=MAT,ymin = fit-se.fit, ymax = fit+se.fit), fill = "#701F57FF", alpha = .4,color = NA)+
  scale_y_continuous(labels = label_number(accuracy = 0.1),limits=c(-1,2))
    
png("figures/PD/Temperate_MAT*dist.ports_wpoints.jpg", width = 6, height = 6, units = 'in', res = 300)
diff.MAT_dist.ports_resid 
dev.off()

diff.MAT_dist.ports_resid.low <-
  ggplot(data = resid.low, aes(x = MAT, y = resid.d.mntd, color = "#F6A47BFF"))+
  ...
  theme(legend.position = "none", 
        legend.text = element_text(size = 15),
        legend.title = element_text(size = 15))+
  geom_ribbon(data = pdat.low,aes(y=NULL,x=MAT,ymin = fit-se.fit, ymax = fit+se.fit), fill = "#F6A47BFF", alpha = 0.6,color = NA)+
  geom_line(data = pdat.low, aes(x = MAT, y = fit), color = "black", size = 1.5, alpha = 1) +
  scale_y_continuous(labels = label_number(accuracy = 0.1),limits=c(-1,1.5))
  
png("figures/PD/Temperate_MAT*dist.ports_wpoints.low.jpg", width = 6, height = 6, units = 'in', res = 300)
diff.MAT_dist.ports_resid.low
dev.off()

diff.MAT_dist.ports_resid.high <-
  ggplot(data = resid.high, aes(x = MAT, y = resid.d.mntd, color = "#701F57FF"))+
    ...
    theme(legend.position = "none", 
          legend.text = element_text(size = 15),
          legend.title = element_text(size = 15))+
    geom_ribbon(data = pdat.high,aes(y = NULL,x = MAT,ymin = fit-se.fit, ymax = fit + se.fit), fill = "#701F57FF", alpha = 0.8,color = NA)+
    geom_line(data = pdat.high, aes(x = MAT, y = fit),color = "black", size = 1.5, alpha = 1) +
    scale_y_continuous(labels = label_number(accuracy = 0.1),limits=c(-1,1.5))

png("figures/PD/Temperate_MAT*dist.ports_wpoints.high.jpg", width = 6, height = 6, units = 'in', res = 300)
diff.MAT_dist.ports_resid.high
dev.off()
...
png("figures/PD/diff_forestplot.jpg", width = 8, height = 6, units = 'in', res = 300)
forest.plot
dev.off()

#####################################
######## Prop bias simulation #######
######### Beaury et al 2020 #########
#####################################

#set up blank plot to fill in with model predictions
plot(0,0, type="n",lwd=2, ylim=c(0,0.3), xlim=c(0,450),xlab="nat",ylab="prob(nonnat)")
abline(h=0.1, lwd=2, col=2)
#simulate data using range of total richness from observed data and average proportion of native species per plot (0.9)
for(i in 1:500){
  #simulated Total richness:
  n <- 50
  tot <- sort(sample(3:441, n, replace=T))
  ...
  #plot
  lines(predict(denom, type="response") ~ nat, type="l",lwd=2, ylim=c(0,1), col=adjustcolor("gray",0.05))
}
