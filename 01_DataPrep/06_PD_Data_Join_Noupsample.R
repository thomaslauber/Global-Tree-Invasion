...
pdmet.inv <- readRDS("data/PDmetrics_Allspecies_noupsample.rds") %>%                                                         # Read all phy data
  spread(type, value) %>%                                                                                              # Spread data
  rename(plot_id = plot, all.faith = faith, all.mntd = mntd, all.mpd = mpd, all.vntd = vntd, all.vpd = vpd)            # Rename cols

pdmet.nat <- readRDS("data/PDmetrics_Nativespecies_noupsample.rds") %>%                                                      # Read native phy data
  spread(type, value) %>%                                                                                              # Spread dat
  rename(plot_id = plot, nat.faith = faith, nat.mntd = mntd, nat.mpd = mpd, nat.vntd = vntd, nat.vpd = vpd)            # Rename cols

#####################################
###### Join PD nat & invasive #######
#####################################

pdmet <- pdmet.nat %>%                                                                                                 # Start with pdmet.nat
  left_join(pdmet.inv, by = c("plot_id")) %>%                                                                          # Join dataframes
...
#join pdmet with comp.static and lat
pdmet.env.comp <- lat %>%                                                                                               # Start with lat 
  left_join(pc, by = c("plot_id")) %>%                                                                                  # Join with invasion dat
  left_join(comp.human, by = "plot_id") %>%                                                                             # Join with comp.human
  left_join(pdmet, by = "plot_id") %>%                                                                                  # Join with fdmet
...
dat <- pdmet.env.comp %>%                                                                                    # Start pdmet.env.comp
...
pdmet.env.comp.simp <-  pdmet.env.comp %>%                                                                   # Start pdmet.env.comp
  select(lat, avglon, biome, biome2)                                                                            # Select variables of interest

dat.final.analyses <- dat %>%
  left_join(pdmet.env.comp.simp, by = c('lat','avglon')) %>%
  distinct()

write.csv(dat.final.analyses, "data/GFBI_invasion_PDforanalyses_noupsample.csv")
saveRDS(dat.final.analyses, "data/GFBI_invasion_PDforanalyses_noupsample.rds")