...
#########################################
outputPath = 'ninimalina/Global_Invasion/output'
dataPath = 'ninimalina/Global_Invasion/01_InputData/'
glonafPath = dataPath + 'GloNAF'
kewPath = dataPath + 'Kew'
GFBIPath = dataPath + 'GFBI'
BCGI_path = 'Y:/Niamh/02_Data/BCGI_GFBI_trees'

GFBI_file = 'GFBI_fixed_plots_acceptname_avglatlon_plotyear_min3.csv'

BCGI_treeList = 'BCGI_tree_list.csv'

assetName = GFBI_uniquePlots_File[:-4]

bucket = 'niamh_bucket_31415927'
formattedBucket = 'gs://' + bucket
GCSB_subfolder = 'GFBI_Glonaf_Kew/'

GEE_path = 'users/robmannn/GFBI_Glonaf_Kew/'
polygonPathGEE_Glonaf = 'users/robmannn/Glonaf/'
polygonName_Glonaf = 'GlonafPolygonsJune2021'
polygonPathGEE_Kew = 'users/robmannn/kew_database/'
polygonName_Kew = 'level3'
kew_prop = 'LEVEL3_COD'
...
#########################################
# bash function
bashFunction_EarthEngine = 'earthengine'
...
GFBI_data = pd.read_csv(GFBIPath + '/' + GFBI_file, chunksize=10000)
...
uniqueCoords.to_csv(dataPath + '/' + 'GFBI_plotCoords.csv', header = True, index= False)

# Normal clean-up of GFBI data: 
    # keep only latest year entries per plot_id
...
GFBI_data_cleanedYear['name_check'] = GFBI_data_cleanedYear.apply(lambda x: 'spp' if len(x['accepted_bin'].split(' '))==2 else 'Genus only', axis=1)
...
BCGI_treesDF = pd.read_csv(BCGI_path + '/' + BCGI_trees, sep=';')
...
downloadBucket = ['gsutil', '-m', 'cp', '-r', downloadBucket, dataPath]
...
GFBI_key = GFBI_data_cleanedYear.merge(keyTable, how='left', left_on='plot_id', right_on='GFBI_plot_id').reset_index(drop=True)
